...
# Wywołanie skryptu
# python scenariusz_5.py -dbconf X -dset Y
#   -dbconf X
...
stmt = "UPDATE title_ratings " \
       "SET averagerating = ROUND(DBMS_RANDOM.VALUE(5, 10), 1), votesnumber = FLOOR(DBMS_RANDOM.VALUE(1000, 50000)) " \
...
print('Scenariusz 5')
print('Opis: ')
print('Aktualizowanie wielu rekordów z tabeli title_ratings z wieloma warunkami (zastosowanie klauzuli WHERE) '
      '- aktualizowane są rekordy, które w kolumnie averageRating mają mniejszą wartość niż 5 (averageRating < 5) '
      'oraz w kolumnie votesNumber mają większą wartość niż 1000 (votesNumber > 1000). '
      'Modyfikacja wszystkich kolumn z tabeli title_ratings, poza kolumną określoną jako klucz główny tabeli. '
      'Wartości w kolumnach: averageRating są wypełniane wygenerowanymi liczbami pseudolosowymi (zmiennprzecinkowymi) z przedziału (5, 10), '
      'zaokrąglanymi do 1 miejsca po przecinku, votesNumber są wypełniane wygenerowanymi liczbami pseudolosowymi (całkowitymi) z przedziału (1_000, 50_000).')
...
for i in tqdm(range(100)):
    ...
    title_ratings.to_sql('title_ratings', con=engine, schema='C##ADMIN', if_exists='append', index=False,
