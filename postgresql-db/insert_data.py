...
with engine.begin() as conn:
    ...
    conn.execute(text('TRUNCATE TABLE title_crew CASCADE'))

    print('Deleting data from table: crew_member ...')
    conn.execute(text('TRUNCATE TABLE crew_member CASCADE'))

    print('Deleting data from table: crew_role ...')
    conn.execute(text('TRUNCATE TABLE crew_role CASCADE'))

    print('Deleting data from table: title_genres ...')
    conn.execute(text('TRUNCATE TABLE title_genres CASCADE'))

    print('Deleting data from table: genre ...')
    conn.execute(text('TRUNCATE TABLE genre CASCADE'))
    ...
    conn.execute(text('TRUNCATE TABLE title CASCADE'))

    print('Deleting data from table: title_type ...')
    conn.execute(text('TRUNCATE TABLE title_type CASCADE'))
...
title_type.to_sql('title_type', con=engine, schema='public', if_exists='append', index=False, method='multi')
...
for title_chunk in title:
    ...
    title_chunk['forAdults'] = title_chunk['forAdults'].apply(lambda x: bool(int(x)))
    ...
    title_chunk.to_sql('title', con=engine, schema='public', if_exists='append', index=False, method='multi')
    pbar.update(1)
pbar.close()
...
genre.to_sql('genre', con=engine, schema='public', if_exists='append', index=False, method='multi')
...
for title_genres_chunk in title_genres:
    title_genres_chunk.to_sql('title_genres', con=engine, schema='public', if_exists='append', index=False, method='multi')
    pbar.update(1)
pbar.close()
...
for crew_member_chunk in crew_member:
    ...
    crew_member_chunk.to_sql('crew_member', con=engine, schema='public', if_exists='append', index=False, method='multi')
    pbar.update(1)
pbar.close()
...
crew_role.to_sql('crew_role', con=engine, schema='public', if_exists='append', index=False, method='multi')
...
for title_crew_chunk in title_crew:
    title_crew_chunk.to_sql('title_crew', con=engine, schema='public', if_exists='append', index=False, method='multi')
    pbar.update(1)
pbar.close()
...
for title_ratings_chunk in title_ratings:
    ...
    title_ratings_chunk.to_sql('title_ratings', con=engine, schema='public', if_exists='append', index=False, method='multi')
    pbar.update(1)
pbar.close()
